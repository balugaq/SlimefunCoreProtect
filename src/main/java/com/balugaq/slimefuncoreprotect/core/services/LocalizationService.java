package com.balugaq.slimefuncoreprotect.core.services;

import com.balugaq.slimefuncoreprotect.api.Language;
        ...
        do {
            if (!languages.hasNext()) {
                if (!FAILED_PATHS.contains(path)) {
                    SlimefunCoreProtect.getInstance().getLogger().severe("No localization found for path: " + path);
